...
\mtjnote{This idea is also described in Random Zoo. He further assumes, that smart users will be as close to deadline as possible.}
...
\mtjnote{Unicorn says to input as close to deadline as possible. We do not think this is good because congestion of many inputs --- besides, the beacon will not receive many messages at the start of input collection (idle time). We want to spread it out. We don't want them to worry about deadlines, because we cannot guarantee deadlines anyway. As such, users can input whenever they want. It is clear, that waiting 1 minute for a commit and waiting 5 seconds is not the same. }