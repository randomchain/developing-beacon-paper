...
\mtjnote{Needs read through + maybe more descriptive title}
We designed our beacon to be run by a single operator, and we considered the possibilities of the operator being malicious and users losing confidence in an operator.
...
One could also question what the motivation to run the beacon is, other than offering a public good. We have not implemented any monetary incentives to run the beacon, meaning it will operate at a deficit to the operator. One incentive could be to obtain secure public randomness for their own use \mtjnote{I do not agree --- we argued in the intro, that for own use, they should just use urandom} \srknote{For their own use = to agree on randomness with others}.
