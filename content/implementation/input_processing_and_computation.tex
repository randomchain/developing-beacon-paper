...
\subsection{Summary}
\mtjnote{this is not a summary:}
Together, these components form a working \acrshort{poc} implementation of a randomness beacon.
The beacon approximates the design outlined in \cref{sec:design}, while making some practical compromises, such as having duplex communication between the input processor and the computation nodes. \stefan{What are the hard gurantees you give with your beacon? Promised probabilistic guarantees in the beginning, what did you mean by that? Get back to it on this occasion}
