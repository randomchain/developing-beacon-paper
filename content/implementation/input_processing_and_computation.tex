...
\msmnote{Might be too much to namedrop bitcoin just for merkle trees?}
...
\subsection{Summary}
Together, these components form a working \acrshort{poc} implementation of a randomness beacon. The beacon approximates the design outlined in \cref{sec:design}, while making some practical compromises, such as having duplex communication between the input processor and the computation nodes.

Moreover, the reliability of the beacon could be greatly improved by eliminating single points of failure, such as the proxies and input processor.
