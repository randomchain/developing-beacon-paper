...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}[auto, remember picture]
        ...
        \draw (ic2) edge[out=0, in=180, arrow] node[below, inner sep=1pt]{$H(i)$} ([yshift=0em]proxy1.west);
        \draw (ic3) edge[out=0, in=180, arrow] node[below, inner sep=1pt]{$H(i)$}([yshift=-.8em]proxy1.west);
        ...
        \draw ([yshift=.8em]ip.east) edge[out=180, in=0, arrow, bend right=25, postaction={decorate,decoration={raise=0.5ex, text along path, text align=center, text={2. Data}}}] ([yshift=-.1em]comp1.west);

        \draw ([yshift=.1em]comp2.west) edge[out=0, in=180, arrow, bend right=20]([yshift=.1em]ip.east);
