...
\mtjnote{Write something about our scoring --- e.g.\ that 3 in damage is a user using a biased output.}
...
These threats are far more damaging if not detected. Where availability is binary and users obviously cannot use a missing output, successful integrity attacks provide an output, that ideally appears legitimate, but is biased. As this arguably is the single most important property of a randomness beacon, we rate all of these as high damage. \mtjnote{Write something more general about our scoring}
...
\mtjnote{TODO: Write and make references to \cref{app:dread} (not necessarily just in this summary)}