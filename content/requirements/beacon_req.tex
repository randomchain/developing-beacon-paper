...
This benefits the integrity of the beacon as some previously mentioned attacks would be detected in this setting \mtjnote{but isn't this one of our nice surprises for later? It seems a bit anti-climatic to just casually mention it here}.
...
The input used, output, and any data needed for verification of an output should be enforced to be published as soon as possible to make the beacon more transparent.
By having a requirement of timeliness at the protocol level, we restrict the time a malicious operator has available to diverge from protocol before users will suspect him.
...
Should a single channel be attacked, input could still be submitted to another. This requirement further increases usability since different users may prefer certain input and output channels.
...
\msmnote{Maybe include non-requirements. Hmm not sure} \mtjnote{I do not think it makes sense here, but the discussions about why we do not sign etc.\ should possibly be elsewhere in the paper.}
