...
As such, the users need to blindly trust the beacon operator \mtjnote{Insert source} --- an assumption we do not want to make.
...
As long as the output is unpredictable for all parties including the beacon operator, we do not really need randomness, and the beacon output can be produced by a deterministic algorithm.
In a sense, the word \enquote{randomness} is misleading in our definition.
No randomness is necessary but we care deeply about unpredictability.\msmnote{that's a bold strategy}
