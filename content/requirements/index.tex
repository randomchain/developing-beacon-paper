...
Problems: The sources dictate the output rate, and sources can be compromised, and provides no guarantee / does not really solve the issue (the users should also make a bitcoin transaction, have a stock company, or observe the drawing from the lotteries to make sure everybody is not colluding against them); although it is far more unlikely that any adversary will be able to control stock markets, lotteries, and the entire bitcoin network.
    ...
    \rene{Consider: ideal situation vs. "good enough"}
...
This seems like a reasonable assumption \rene{I guess that depends on the use-case; it also carries the risk of obvious DoS attacks}.
...
\rene{Remember: Security Rule \#1: KISS (complexity kills security); I would recommend looking at the design/architecture of  "qmail"}
...
\rene{Maybe worth noting that the input can be (truly) random?}
...
In practice this should not be a major problem, since input can be contributed over an insecure channel (they are not secret), and likewise the output can be retrieved over an insecure channel \rene{What about integrity of the submitted input? Is probably also covered by re-computing the result, but need an argument} because the correctness of computation can be verified.
\end{description}
